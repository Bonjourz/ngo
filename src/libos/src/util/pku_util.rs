...
pub fn set_pku_enabled() {
    assert_eq!(false, PKU_ENABLED.load(Ordering::SeqCst));
    ...
    // Alloc pkey
    let mut userspace_pkey = -1;
    let sgx_status = unsafe { occlum_ocall_pkey_alloc(&mut userspace_pkey, 0, 0) };
    assert!(sgx_status == sgx_status_t::SGX_SUCCESS && userspace_pkey == PKEY_USER);
